...
public class WiredMBusConnection {
    ...
    public static TreeMap<String, String> read(Conf meterConf) throws IOException {
                    ...
                    if (variableDataStructure == null) {
                        ...
                        try {
                            ...
                            for (DataRecord data : dataRecords) {
                                ...
                                byte[] dib = data.getDib();
                                String dibStr = bytesToHex(dib);
                                String vibCommaDibStr=vibStr+","+dibStr;
                                ...
                                for (MeterItem item : meterConf.getMeterItems()) {
                                    if (vibCommaDibStr.equals(item.itemCode)) {
        ...
        for (MeterItem item : meterConf.getMeterItems()) {
            if (map.get(item.meterName + "__" + item.description) == null) {
                map.put(item.meterName + "__" + item.description, "NULL");
            }
        }
        return map;
    }
