package org.openhab.binding.gridpalWireless.internal;
...
import org.openhab.binding.gridpalWireless.WirelessConstants;
import org.openhab.binding.gridpalWireless.agent.Agent;
...
public class WirelessUtility {
    ...
    /**
     * Connects to a previously connected connection. In this case, password is not required. Also, properly handles
     * error conditions.
     * 
    ...
    public void Connect(String ssid) throws FailedLoginException, IllegalArgumentException, IllegalStateException {
        ...
        for (int i = 0; i < activeConnections.length(); i++) {
            ...
            if (connection.getString("type").equals(WirelessConstants.WIRELESS_DEVICE_TYPE)
                    && connection.getString("name").equals(ssid)) {
    ...
    public void Connect(String ssid, String password)
            throws FailedLoginException, IllegalArgumentException, IllegalStateException {
        ...
        String cmd = String.format("nmcli %s device wifi connect '%s' password '%s'", WirelessConstants.NMCLI_OPTS,
                ssid, password);
        String result = agent.Execute(cmd);
    ...
    public boolean Disconnect() throws IllegalStateException {
        ...
        for (int i = 0; i < activeConnections.length(); i++) {
            ...
            if (connection.getString("type").equals(WirelessConstants.WIRELESS_DEVICE_TYPE)) {
                ...
                String cmd = String.format("nmcli %s connection down '%s'", WirelessConstants.NMCLI_OPTS,
    ...
    public JSONArray GetConnections() {
        ...
        try {
            ...
            String cmd = String.format("nmcli %s -f %s connection show", WirelessConstants.NMCLI_OPTS,
                    String.join(",", FIELDS));
    ...
    public JSONArray GetAvailableConnections() {
        ...
        try {
            ...
            cmd = String.format("nmcli %s -f %s device wifi list", WirelessConstants.NMCLI_OPTS,
                    String.join(",", FIELDS));
            ...
            for (String line : result.split("\n")) {
                ...
                try {
                    ...
                    if (index > -1) {
                        ...
                        if (Integer.valueOf(json.getString(FIELDS[1])) > Integer
                                .valueOf(connection.getString(FIELDS[1]))) {
                            logger.debug("Higher signal strength found. Replacing old value '{}' with new value '{}'.",
                                    connection.getString(FIELDS[1]), json.getString(FIELDS[1]));
    ...
    public JSONArray GetActiveConnections() {
        ...
        try {
            final String[] FIELDS = { "name", "type", "device", "uuid" };
            String cmd = String.format("nmcli %s -f %s connection show --active", WirelessConstants.NMCLI_OPTS,
                    String.join(",", FIELDS));
    ...
    public JSONObject GetWirelessStatus() {
        ...
        String cmd = String.format("nmcli %s -f %s general status", WirelessConstants.NMCLI_OPTS,
                String.join(",", FIELDS));
    ...
    public void DeleteConnection(String name) {
        ...
        for (int i = 0; i < connections.length(); i++) {
            ...
            if (connection.getString("type").equals(WirelessConstants.WIRELESS_DEVICE_TYPE)
                    && connection.getString("name").equals(name)) {
